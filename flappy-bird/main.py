...
scoreSilver = 20
scoreGold = 30
deathscreenScoreBG = pygame.image.load("img/deathscreenScoreBG.png").convert_alpha()
deathscreenSilver = pygame.image.load("img/silber.png").convert_alpha()
deathscreenBronce = pygame.image.load("img/bronze.png").convert_alpha()
deathscreenGold = pygame.image.load("img/gold.png").convert_alpha()
deathscreenBtn = pygame.image.load("img/deathscreenBtn.png").convert_alpha()
deathscreenYouLost = pygame.image.load("img/deathscreenYouLost.png").convert_alpha()
...
def deathscreenDefault(): 
    global best, gameActive
    if best < score: best = score
    screen.fill((0,255,0))
    imgGold = deathscreenGold.copy()
    imgSilver = deathscreenSilver.copy()
    imgBronce = deathscreenBronce.copy()
    if score < scoreBronce: imgBronce.fill((255,255,255,70), special_flags=pygame.BLEND_RGBA_MULT)
    if score < scoreSilver: imgSilver.fill((255,255,255,70), special_flags=pygame.BLEND_RGBA_MULT)
    if score < scoreGold: imgGold.fill((255,255,255,70), special_flags=pygame.BLEND_RGBA_MULT)
    screen.blit(deathscreenYouLost, (200, 100))
    screen.blit(deathscreenScoreBG, (200, 200))
    scorestr = font.render(str(score), True, (0, 0, 0))
    beststr = font.render(str(best), True, (0, 0, 0))
    ...
    screen.blit(deathscreenBtn, (200, 420))
    if pygame.Rect((200, 420),deathscreenBtn.get_size()).collidepoint(pygame.mouse.get_pos()):
